...
> **Status: M4 (gate) — the portal plays PolyTrack end-to-end.** The loader, mappings, AST-transform pipeline, and portal (proxy + service worker) are all built and unit-tested (**115 tests**, CI green). A transformed, modded PolyTrack now boots, clears the game's "unofficial version" gate, loads a track, and reaches **actual playable gameplay** (verified headlessly — a race on "Summer 1" with full HUD). Only online/leaderboard access (issue #7, M8) remains rough. Next: **M4 continues** — event bus + API bridge so real mods can hook the now-playable game. See [`docs/project/progress.md`](./docs/project/progress.md) and [`docs/research/portal-browser-test-findings.md`](./docs/research/portal-browser-test-findings.md).
