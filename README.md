...
> - **`/browse` lists all 20 mods from PML's own registry** alongside the native ones, each tagged with the loader that runs it (`tspml`/`pml`) — a derived chip that is also a real filter. Summaries were written from each mod's source (PML's registry carries none), and the six with no 0.6.2 build say so on the card.
...
>
> **1,262 unit tests + 12 CI smokes, all green.** Headlessly verified against the real game via Playwright — including a fixture PML mod imported from its own CDN layout and run end to end. See [`docs/project/progress.md`](./docs/project/progress.md).
