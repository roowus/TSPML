...
**Cleanup is implemented** (#17) — `LoadResult.unload()` disposes every loaded mod in **reverse** load order, isolated per mod, idempotent, and awaited. A class-form mod implements `onUnload(api)`; a factory-form mod returns a disposer. Note the split: the loader *calls* mod cleanup, but does not *emit* `loader.onUnload` — `ModApi.events` is `on`/`off` only, so emitting is the host's job. The portal does that job in `lib/teardown.ts`, on React unmount **and** `pagehide`, emitting `loader.onUnload` *before* anything is torn down so a handler still has a live bridge to release against; the portal smoke asserts a real page teardown actually runs a real mod's disposer. See [events-and-registries.md](../api/events-and-registries.md#cleanup-how-a-mod-actually-unloads).
