...
| **M3 — AST transform + resolver** | 🟡 spike ✅ VIABLE / pipeline ⏳ | **Spike: GO** — Babel modifies the real 0.6.2 bundle, `node --check` passes, module-map 211==211 ([report](../research/transform-spike.md)). Pipeline next: mixin-op API resolved via mappings, symbol-level locators, source maps, per-chunk. |
