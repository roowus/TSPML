...
```sh
pnpm test    # 53 unit tests (diff + verify-targets + fetch + wasm-locate) — CI-runnable, no bundle needed
```

The pure `diff`, `verify-targets`, `fetch` and `wasm-locate` logic is fully unit-tested with fixture maps and
